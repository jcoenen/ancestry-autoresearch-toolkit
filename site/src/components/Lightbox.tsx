...
export default function Lightbox({ items, currentIndex, isOpen, onClose, onNext, onPrev }: LightboxProps) {
  ...
  const isPdf = item?.path.toLowerCase().endsWith('.pdf') ?? false
      ...
      <div className="relative z-[1] flex items-center justify-center w-full h-full px-12 sm:px-20 pt-16 pb-28">
        ...
        {isPdf ? (
          <div className="flex flex-col items-center justify-center gap-4 text-white/80">
            ...
            <a href={url} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-sm font-medium transition-colors">
              Open PDF
            </a>
          </div>
        ) : (
